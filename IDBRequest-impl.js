const EventTargetImpl =
	require("./living/idl/EventTarget-impl.js").implementation;
const DOMException = require("domexception/lib/DOMException");

const {
	setupForSimpleEventAccessors,
} = require("./living/helpers/create-event-accessor");
...
class IDBRequestImpl extends EventTargetImpl {
	constructor(
		globalObject,
		args,
		{ parent, transaction = null, source = null },
	) {
	...
	get error() {
		if (this.readyState !== "done") {
			throw DOMException.create(
				this._globalObject,
				[
					"Cannot access the error property. The request has not completed yet. Wait for the request to finish before accessing the error property.",
					"InvalidStateError",
				],
				{},
			);
	...
	get result() {
		if (this.readyState !== "done") {
			throw DOMException.create(
				this._globalObject,
				[
					"Cannot access the result property. The request has not completed yet. Wait for the request to finish before accessing the result property.",
					"InvalidStateError",
				],
				{},
			);
