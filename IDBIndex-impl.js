...
class IDBIndexImpl {
	...
	set name(to) {
		...
		if (this._index.name !== to) {
			...
			this.objectStore._schema.renameIndex(
				this.objectStore._store.id,
				this._index.name,
				to,
			);
	...
	_getAll(query, count, keys) {
		...
		const request = createImpl(
			this._globalObject,
			{},
			{ source: this, transaction: this.objectStore._transaction },
		);
		this.objectStore._transaction._queue.push({
			count: count,
			index: this._index,
