...
class IDBObjectStoreImpl {
	...
	constructor(
		globalObject,
		_args,
		{ transaction, schema, name, constructing = false },
	) {
	...
	set name(to) {
		if (this._isDeleted()) {
			throw DOMException.create(
				this._globalObject,
				["The object store has been deleted.", "InvalidStateError"],
				{},
			);
		}
		if (this._transaction.mode !== "versionchange") {
			throw DOMException.create(
				this._globalObject,
				[
					"The object store name can only be changed during a versionchange transaction.",
					"InvalidStateError",
				],
				{},
			);
		...
		if (this._transaction._state !== "active") {
			throw DOMException.create(
				this._globalObject,
				["The transaction is not active.", "TransactionInactiveError"],
				{},
			);
		}
		if (this._store.name !== to) {
			if (this._schema.getObjectStore(to) !== null) {
				throw DOMException.create(
					this._globalObject,
					[
						`An object store with the name "${to}" already exists.`,
						"ConstraintError",
					],
					{},
				);
	...
	get indexNames() {
		return DOMStringList.create(this._globalObject, [], {
			array: this._schema.getIndexNames(this._store.name),
		});
	}
	...
	_addOrPut(value, key, overwrite) {
		...
		if (this._transaction.mode === "readonly") {
			throw DOMException.create(
				this._globalObject,
				[
					"The transaction is in readonly mode. Cannot write to a readonly transaction.",
					"ReadOnlyError",
				],
				{},
			);
		...
		if (key !== null && this._store.keyPath !== null) {
			throw DOMException.create(
				this._globalObject,
				[
					"The object store uses a key path and no separate key should be provided.",
					"DataError",
				],
				{},
			);
		...
		if (
			key === null &&
			this._store.autoIncrement === null &&
			this._store.keyPath === null
		) {
			throw DOMException.create(
				this._globalObject,
				[
					"The object store requires a key, but none was provided.",
					"DataError",
				],
				{},
			);
	...
	delete(query) {
		...
		if (this._transaction.mode === "readonly") {
			throw DOMException.create(
				this._globalObject,
				[
					"The transaction is in readonly mode. Cannot delete from a readonly transaction.",
					"ReadOnlyError",
				],
				{},
			);
		...
		if (query !== null && !(query instanceof this._globalObject.IDBKeyRange)) {
			query = this._globalObject.IDBKeyRange.only(
				convert.key(this._globalObject, query),
			);
		}
		const request = IDBRequest.createImpl(this._globalObject, [], {
	...
	clear() {
		...
		if (this._transaction.mode === "readonly") {
			throw DOMException.create(
				this._globalObject,
				[
					"The transaction is in readonly mode. Cannot clear a readonly transaction.",
					"ReadOnlyError",
				],
				{},
			);
	...
	_get(query, keys) {
		...
		if (query === null) {
			throw DOMException.create(
				this._globalObject,
				["The query parameter cannot be null.", "DataError"],
				{},
			);
		}
	...
	index(name) {
		...
		if (this._transaction._state === "finished") {
			throw DOMException.create(
				this._globalObject,
				[
					"The transaction has finished. Cannot access indexes from a finished transaction.",
					"InvalidStateError",
				],
				{},
			);
		...
		if (index === null) {
			throw DOMException.create(
				this._globalObject,
				[
					`The index "${name}" was not found in the object store "${this._store.name}".`,
					"NotFoundError",
				],
				{},
			);
	...
	createIndex(name, keyPath, { unique = false, multiEntry = false } = {}) {
		...
		if (this._transaction.mode !== "versionchange") {
			throw DOMException.create(
				this._globalObject,
				[
					"Indexes can only be created during a versionchange transaction.",
					"InvalidStateError",
				],
				{},
			);
		...
		if (this._store.index[name] !== null) {
			throw DOMException.create(
				this._globalObject,
				[
					`An index with the name "${name}" already exists in the object store "${this._store.name}".`,
					"ConstraintError",
				],
				{},
			);
		...
		if (Array.isArray(keyPath) && multiEntry) {
			throw DOMException.create(
				this._globalObject,
				[
					"The multiEntry option cannot be used with an array keyPath.",
					"InvalidAccessError",
				],
				{},
			);
		}
		const index = this._schema.createIndex(
			this._store.name,
			name,
			keyPath,
			multiEntry,
	...
	deleteIndex(name) {
		...
		if (this._transaction.mode !== "versionchange") {
			throw DOMException.create(
				this._globalObject,
				[
					"Indexes can only be deleted during a versionchange transaction.",
					"InvalidStateError",
				],
				{},
			);
