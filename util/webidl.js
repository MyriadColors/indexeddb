...
const transformer = new Webidl2js({
	...
	processCEReactions(code) {
		const preSteps = this.addImport(
			"../helpers/custom-elements",
			"ceReactionsPreSteps",
		);
		const postSteps = this.addImport(
			"../helpers/custom-elements",
	...
	processHTMLConstructor() {
		const identifier = this.addImport(
			"../helpers/html-constructor",
			"HTMLConstructor",
		);
	...
	processReflect(idl, implObj) {
		...
		const attrName =
			(reflectAttr?.rhs && JSON.parse(reflectAttr.rhs.value)) ||
		...
		if (idl.extAttrs.some((attr) => attr.name === "ReflectURL")) {
			...
			if (
				!isSimpleIDLType(idl.idlType, "USVString") &&
				!isSimpleIDLType(idl.idlType, "DOMString")
			) {
				throw new Error(
					"[ReflectURL] specified on non-USV/DOMString attribute",
				);
			}
			const parseURLToResultingURLRecord = this.addImport(
				"../helpers/document-base-url",
		...
		if (isSimpleIDLType(idl.idlType, "unsigned long")) {
			const parseNonNegativeInteger = this.addImport(
				"../helpers/strings",
...
transformer.generate(outputDir).catch((error) => {
	console.error(error);
	process.exit(1);
});
