require("proof")(1, async (okay) => {
	...
	await harness(async () => {
		...
		open_rq.onsuccess = function onsuccess(_e) {
			...
			idx.getKey("Rita").onsuccess = this.step_func(function onsuccess(_e) {
				gotten_keys.push(e.target.result);
			});
			idx.getKey("Scheeta").onsuccess = this.step_func(function onsuccess(_e) {
				gotten_keys.push(e.target.result);
			});
			idx.getKey("Neil").onsuccess = this.step_func(function onsuccess(_e) {
				gotten_keys.push(e.target.result);
			});
			idx.getKey("Bobby").onsuccess = this.step_func(function onsuccess(_e) {
				gotten_keys.push(e.target.result);
