require('proof')(6, async okay => {
    ...
    await harness(async () => {
        ...
        open_rq.onupgradeneeded = function onupgradeneeded(e) {
            ...
            objStore.add({k:5}).onsuccess = function onsuccess(_e) { keys.push(e.target.result); }
            objStore.add({}).onsuccess = function onsuccess(_e) { keys.push(e.target.result); }
            ...
            objStore2.add("Unicorns'R'us").onsuccess = function onsuccess(_e) { keys.push(e.target.result); };
            assert_false(objStore2.indexNames.contains("idx"), "index exist on new objstore");
            assert_equals(objStore2.keyPath, null, "keyPath");

            assert_throws_dom("NotFoundError", function onupgradeneeded() { objStore2.index("idx"); });
        }
