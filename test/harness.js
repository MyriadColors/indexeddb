module.exports = async (okay, name) => {
    ...
    */
	...
	function assert_not_equals(actual, expected, message) {
		okay(
			actual !== expected,
			`${scope.name} - ${get_assertion_message(message)}`,
		);
	}
	...
	function assert_throws_dom(type, func, description) {
		try {
			...
			const actualErrorName =
				DOM_ERROR_CODE_NAMES[error.code] ||
				error.name ||
			...
			assert_equals(
				...
				description ||
					`Expected ${expectedErrorName} (code: ${expectedCode}), got ${actualErrorName} (code: ${error.code})`,
			);
			...
			return new Promise((resolve) => {
				...
				test.step(() => {
					assert(
						...
						"promise_test",
						null,
						`test body must return a 'thenable' object (received ` +
							String(promise) +
							`)`,
						{ value: promise },
					);
					...
					.catch(
						test.step_func((value) => {
							...
							assert(
								...
								"promise_test",
								null,
								`Unhandled rejection with value: ` + String(value),
								{ value: value },
							);
						}),
					)
					...
					subChecks.push(
						new Promise((resolve, reject) => {
							...
							request.onsuccess = testCase.step_func(() => {
								...
								resolve(
									checkValue(testCase, result, valueDescriptors[requestIndex]),
								);
							});
						}),
					);
