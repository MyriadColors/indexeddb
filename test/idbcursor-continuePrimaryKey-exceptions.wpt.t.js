require('proof')(12, async okay => {
    ...
    await harness(async () => {
        ...
        async_test((t) => {
            ...
            open.onupgradeneeded = t.step_func(function onupgradeneeded() {
                ...
                request.onsuccess = t.step_func(function onsuccess() {
                    ...
                    assert_throws_dom('InvalidAccessError', function onsuccess() {
        ...
        [
            {
                continue_key: 2, continue_primaryKey: 'a', direction: 'nextunique', expected_key: 1, expected_primaryKey: 'a'
            },
            {
                continue_key: 2, continue_primaryKey: 'a', direction: 'prevunique', expected_key: 3, expected_primaryKey: 'a'
            }
        ].forEach((testcase) => {
            async_test((t) => {
                var dbname = document.location + '-' + t.name;
                ...
                open.onupgradeneeded = t.step_func(function onupgradeneeded() {
                    ...
                    store.put({ik: [1,2,3], pk: 'a'}).onerror =
                    t.unreached_func('put should not fail');
                    store.put({ik: [1,2,3], pk: 'b'}).onerror =
                    t.unreached_func('put should not fail');
