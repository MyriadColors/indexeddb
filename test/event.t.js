require("proof")(2, (okay) => {
	...
	targetable.addEventListener("hello", () => {
		throw new Error();
	});

	const event = new object.Event("hello");
	okay(event.type, "hello", "event");
	EventTarget.convert(targetable)._dispatch(
		Event.convert(event),
		false,
		true,
		noop,
	...
	okay(
		Event.convert(event)._legacyOutputDidListenersThrowFlag,
		"legacy output did listeners throw flag",
	);
});
