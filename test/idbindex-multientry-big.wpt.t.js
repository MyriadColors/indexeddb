    ...
    await harness(async () => {
        ...
        open_rq.onsuccess = function onsuccess(_e) {
            ...
            tx.objectStore('store')
              .put(obj, 1)
              .onsuccess = t_add.step_func(function onsuccess(_e)
            {
                assert_equals(e.target.result, 1, "put'd key");
                this.done();
            });

            tx.oncomplete = t_get.step_func(function oncomplete() {
                var idx = db.transaction('store').objectStore('store').index('index')

                for (let i = 0; i < 1000; i++)
                {
                    idx.get('index_no_' + i).onsuccess = t_get.step_func(function onsuccess(_e) {
                        assert_equals(e.target.result.test, "yo");
                    });
                }

                idx.get('index_no_999').onsuccess = t_get.step_func(function onsuccess(_e) {
                    assert_equals(e.target.result.test, "yo");
