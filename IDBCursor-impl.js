...
class IDBCursorImpl {
	constructor(
		globalObject,
		{ type, transaction, store, request, direction, source, query, index },
	) {
	...
	continuePrimaryKey(key, primaryKey) {
		...
		key = key === null ? null : convert.key(this._globalObject, key);
		primaryKey =
			primaryKey === null ? null : convert.key(this._globalObject, primaryKey);
		switch (this._direction) {
			case "next": {
	...
	update(value) {
		...
		if (this._store.keyPath !== null) {
			const key = valuify(
				this._globalObject,
				(this.source.objectStore || this.source)._schema.getExtractor(
					this._store.id,
				)(value),
			);
	...
	delete() {
		...
		this._transaction._queue.push({
			...
			query: this._globalObject.IDBKeyRange.only(
				convert.key(this._globalObject, this._value.key),
			),
			request: request,
			store: JSON.parse(JSON.stringify(this._store)),
		});
